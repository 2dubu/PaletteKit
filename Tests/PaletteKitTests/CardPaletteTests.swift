...
@Suite("CardPalette + SwatchStrategy")
struct CardPaletteTests {
    ...
    func vibrantStrategy() {
        let cp = CardPalette(palette: palette, swatches: fullSwatches, strategy: .vibrant)
    ...
    func contrastStrategy() {
        let cp = CardPalette(palette: palette, swatches: fullSwatches, strategy: .contrast)
    ...
    func mutedStrategy() {
        let cp = CardPalette(palette: palette, swatches: fullSwatches, strategy: .muted)
    ...
    func backgroundIsStrategyIndependent() {
        for s: SwatchStrategy in [.vibrant, .contrast, .muted] {
            let cp = CardPalette(palette: palette, swatches: fullSwatches, strategy: s)
    ...
    func vibrantFallback() {
        ...
        let cp = CardPalette(palette: palette, swatches: partial, strategy: .vibrant)
    ...
    func nilSwatches() {
        let cp = CardPalette(palette: palette, swatches: nil, strategy: .vibrant)
        #expect(cp.center.hex == dominant.hex)
        #expect(cp.edge.hex == darkest.hex)
